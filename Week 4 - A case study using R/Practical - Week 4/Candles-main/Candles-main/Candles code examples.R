...
#   geom_smooth(method = "loess", size = 1.5, colour = "royalblue", fill = "royalblue") +
#   geom_point(alpha = 0.1, colour = "royalblue")+
#   labs(x = "Date (Altered)", y = "Average daily rating (1-5) (Altered)", title = "Top 3 scented candles Amazon reviews 2017-2020 (Altered)")+
#   theme_light()+
...
#   geom_vline(xintercept = as.numeric(as.Date("2020-01-20")), colour = "indianred1", linetype = "dashed")+
#   geom_smooth(method = "loess", size = 1.5, colour = "orange", fill = "orange") +
#   geom_point(alpha = 0.1, colour = "orange")+
#   labs(x = "Date (Alt.)", y = "Average daily rating (1-5) (Alt.)", title = "Top 3 unscented candles Amazon reviews 2017-2020 (Alt.)")+
#   theme_light()+
#   theme(plot.title = element_text(size=16))+
#   scale_x_date(date_labels = "%m-%Y", date_breaks = "6 month")
# us1720
...
s5r <- ggplot(s5, aes(x=as.factor(month), y = nsprop, group = month))+
  ...
  labs(x = "Month (Alt.)", y = "Proportion of reviews(Alt.)", title = "Top 5 scented candles on Amazon: \nProportion of reviews mentioning lack of scent by month 2020 (Alt.)")+
  theme_light()+
  theme(plot.title = element_text(size=16))
s5r
