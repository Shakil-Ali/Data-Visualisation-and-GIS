...
>  5075 rows, 5 columns, vector types: int, chr, Factor
...
```{r}
y <- unique(naval_promotions$id)
length(y)
```

(@) Why is that number different than the number of rows?

> Because the same person can be promoted numerous times
...
```{r}
x <- unique(naval_promotions$date, na.rm=TRUE)
earliest <- head(sort(x), 1)
latest <- tail(sort(x), 1)
earliest
latest
```
...
```{r}
my_class <- list(
  footballclubname = "Chelsea FC",
  year_founded = 1905,
  stadium = c("Stamford Bridge", "Fulham Road"),
  manager = "Thomas Tuchel"
)
str(my_class)
is.list(my_class)
is.data.frame(my_class)
```
...
```{r}
prom_cap <- naval_promotions[(naval_promotions$rank) == "captain", ]
head(prom_cap, 10)
```
...
```{r}
prom_1800 <- naval_promotions[(naval_promotions$date) < 1900,]
head(prom_1800, 10)
```
