...
```{r}
paste("Hello", "everybody","my name is Shakil")
```

(@) Now explain in a sentence what happened.

> It concatenatres the strings and adds white space before the string after the comma (,)
...
> The '<' checks if a quantity is less than another quantity
> The '>' checks if a quantity is more than another quantity
> The '==' checks if a quantity is equal to another quantity
...
> Yes, its N/A, because you cannot add 2 to a value which is N/A
...
> Variable='computer_name', Assignment Operator='<-', Value="HAL 9000"
...
> A variable named 'x' got assigned the value 2
> A variable named 'y' got assigned the value 5
> The 'x' and 'y' variables got multiplied
> The variable 'x' got the value 9 added to it 
...
> 20. I was right. This is because it is assigned the variable 'b', whose value is 20
...
> every item in 'many' was multiplied by 2
...
> every value in 'many' has the value 2 added to it

(@) Can you create a variable containing several historical names as a character vector?

```{r}
historical_names <- c("Muhammad (SAW)", "Isa (AS)", "Musa (AS)", "Ibrahim (AS)", "Nuh (AS)")
historical_names
```
...
> Because you cannot add two vectors of different lengths
...
> Checking each book price in 'book_prices', to see if it costs more/less than the mean of the book prices
...
> Missions carried out on churches in the states, their locations and start/end date 
...
```{r}
paulist_missions$converts
paulist_missions$confessions
```

(@) What was the mean number of converts? the maximum? 

```{r}
mean_conv <- mean(paulist_missions$converts)
mean_conv
max_conv <- max(paulist_missions$converts)
max_conv
```
...
```{r}
mean_conv1 <- mean(paulist_missions$confessions, na.rm = TRUE)
mean_conv2 <- mean(paulist_missions$conversions, na.rm = TRUE)
ratio <- mean_conv2/mean_conv1
ratio
```
...
> Can see a positive correlation. Indicates that as conversions increase, the number of conversions also increase
...
library(historydata)
summary(cars)
summary(catholic_dioceses)
plot(catholic_dioceses$lat, catholic_dioceses$long)
title("Latitude versus Longitude")
```
